            ...
            >
                ...
                <div className="relative mb-6">
                    ...
                    <div className="flex flex-col gap-2">
                        <div className="flex items-center gap-1.5 text-royal-gold text-xs relative group/rating">
                            <div className="flex gap-0.5">
                                {[1, 2, 3, 4, 5].map((s) => (
                                    <Star
                                        key={s}
                                        className={`w-3.5 h-3.5 ${s <= Math.round(stats.avg) ? 'fill-royal-gold' : 'opacity-20'}`}
                                    />
                                ))}
                            </div>
                            <span className="text-royal-white/40 ml-1 bg-royal-gold/5 px-2 rounded-md border border-royal-gold/10">
                                {stats.avg} {stats.count > 0 && `(${stats.count} reviews)`}
                            </span>
                        </div>
                        <button
                            onClick={() => setIsReviewOpen(true)}
                            className="text-[10px] text-left text-royal-gold/60 uppercase tracking-widest hover:text-royal-gold underline decoration-dotted underline-offset-4"
                        >
                            Rate & Review
                        </button>
                    </div>
